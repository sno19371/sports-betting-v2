...
class NFLDataSources:
    ...
    def get_player_props_odds(self):
        """Get player prop odds (if available in your plan)"""
        if not ODDS_API_KEY:
            return None
            
        try:
            url = f"{API_URLS['odds_api']}/odds/?apiKey={ODDS_API_KEY}&regions=us&markets=player_props"
            response = requests.get(url, timeout=15)
            self._rate_limit('odds_api')
            ...
            if response.status_code == 200:
                ...
                logger.info("Player props not available in free tier")
                return None
