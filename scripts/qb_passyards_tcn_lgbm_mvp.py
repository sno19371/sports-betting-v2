# qb_passyards_tcn_lgbm_mvp.py
# MVP: 6-game Pass-TCN -> LightGBM for QB passing yards (Windows-friendly, leak-safe)
...
PARQUET_PATH = os.environ.get(
    "PARQUET_PATH",
    r"C:\Users\Alex\Desktop\sports-betting-v2\data\processed\full_games_with_tds_ints_fumbles_yards.parquet"  # <-- change if not using env var
)
ARTIFACT_DIR = "artifacts_qb_pass_mvp"
...
# Year splits
# Encoder split (keeps 2023/2024 unseen while choosing the encoder)
...
log = logging.getLogger("qb_passyards_mvp")
# also log to a file
...
# =========================
# Window builders (carry 'position' into meta)
# =========================
META_BASE_COLS = [
...
def build_windows_crossseason(
    df_qb: pd.DataFrame, feature_cols: List[str], target_col: str, L: int
):
...
def build_windows_withinseason(
    df_qb: pd.DataFrame, feature_cols: List[str], target_col: str, L: int
):
    rows, ys, metas, hist_lengths = [], [], [], []
...
# Ensure 'position' exists in meta (safety net)
...
log.info("LGB train rows: %d | LGB val rows: %d | LGB test rows: %d",
...
with open(os.path.join(ARTIFACT_DIR, "scaler.json"), "w") as f:
    json.dump({"mean": scaler.mean_.tolist(), "scale": scaler.scale_.tolist(),
               "cols": PASS_TCN_COLS, "L": L, "cross_season": CROSS_SEASON_WINDOWS}, f)
...
# LightGBM data
# =========================
# Add history length so trees know early-season windows are thin
meta = meta.copy()
meta["history_length"] = hist_len.astype(np.int16)

CTX_COLS = ["position", "posteam", "opponent", "home_indicator", "season", "week", "history_length"]
ctx = meta[CTX_COLS].copy()
ctx = as_category(ctx, ["position", "posteam", "opponent"])
...
cat_feats = [c for c in ["position", "posteam", "opponent"] if c in X_train.columns]
...
log.info("Training LightGBM head for passing yards.")
...
gbm = lgb.train(
    ...
    callbacks=callbacks,  # use callbacks instead of early_stopping_rounds arg
)
...
gbm.save_model(os.path.join(ARTIFACT_DIR, "lgbm_qb_pass.txt"))
with open(os.path.join(ARTIFACT_DIR, "metrics.json"), "w") as f:
    json.dump({"val_2023": {"mae": mae23, "rmse": rmse23},
               "test_2024": {"mae": mae24, "rmse": rmse24}}, f, indent=2)
